...
func main() {
	bucket := aws.String("test") //bucket名称
	key := aws.String("honglei") //object keyname
	access_key := "QXOGD6FJF7TDKOD70AXY"
	secret_key := "wTddXAwWXtIzelY51Ipo0YhONrghRDo1LZuwejzy"
	end_point := "http://10.0.0.1:7480"            //endpoint设置，不要动
	...
	urlStr, err := req.Presign(time.Minute * 15)
	if err != nil {
		fmt.Println(err.Error())
		return
	}
